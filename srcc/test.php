<?php
#error_reporting(0);
include 'php/Calls.php';
?>

<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Звонки</title>
	<link rel="stylesheet" href="css/proba.css">
	<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/3.5.1/chart.min.js"></script>
	
</head>
<body>
	<h1>Ппроект с проверкой работоспособности ЗВОНКОВ</h1>
	<a href="stat.html">Назад</a>
	<form method="POST">
	    <form id="form" name="form" method="post" action="CheckBox.jsp">
	    	<div style="overflow: auto; width: 400px; height: 80px; border: 1px solid #336699; padding-left: 5px">
		    	<input type="checkbox" name="countries" value="World"> World<br>
		    	<?php
		    		foreach ($u as $item) {
		    			echo "<input type='checkbox' name='countries' value='$item'> $item<br>";
		    		}
		    	?>
		    </div>
		    <br/><input type="button" name="goto" onClick="selectCheckBox()"value="Check">
	    </form>
    	<input type="submit" value="Отправить">
	</form>
	<div class="d">
		<canvas id="myChart" height="400" class="chartCanvas"></canvas>
	</div>
	<script type="text/javascript" >
		var year = <?php echo json_encode($years); ?>; 
		var injuries = <?php echo json_encode($injuries); ?>;
		var name = <?php echo json_encode($selectedOption); ?>;
	</script>
	<script type="text/javascript" src="js/index.js"></script>
</body>
</html>